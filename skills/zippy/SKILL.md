...
The installer also offers to install this coding-agent skill. Following the
[Agent Skills](https://agentskills.io) convention, the files land once in `~/.agents/skills/zippy/`
(the cross-client location) and each detected agent's own skills directory is symlinked at it:
`~/.claude/skills/zippy` (Claude Code), plus `~/.cursor/skills/zippy`, `~/.codex/skills/zippy` and
`~/.windsurf/skills/zippy` when those agents are present. It installs at **user scope**, so the
skill is available in every project. Force it with `--with-skill` (or `ZIPPY_SKILL=1`), skip with
`--no-skill`, or target one agent with `ZIPPY_AGENT=cursor`.

Install the skill on its own at any time:
